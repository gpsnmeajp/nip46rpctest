...
var NostrTools = (() => {
  ...
  var JS = class {
    ...
    verifyEvent(event) {
      ...
      const hash4 = getEventHash(event);
      if (hash4 !== event.id) {
        event[verifiedSymbol] = false;
        return false;
      }
      try {
        const valid = schnorr.verify(event.sig, hash4, event.pubkey);
  ...
  var assert2 = { number: number3, bool: bool2, bytes: bytes3, hash: hash3, exists: exists3, output: output3 };
  ...
  function extract(hash4, ikm, salt) {
    ...
    return hmac2(hash4, toBytes2(salt), toBytes2(ikm));
  }
  var HKDF_COUNTER = new Uint8Array([0]);
  var EMPTY_BUFFER = new Uint8Array();
  function expand(hash4, prk, info, length = 32) {
    assert_default.hash(hash4);
    assert_default.number(length);
    if (length > 255 * hash4.outputLen)
      throw new Error("Length should be <= 255*HashLen");
    const blocks = Math.ceil(length / hash4.outputLen);
    if (info === void 0)
      info = EMPTY_BUFFER;
    const okm = new Uint8Array(blocks * hash4.outputLen);
    const HMAC3 = hmac2.create(hash4, prk);
    const HMACTmp = HMAC3._cloneInto();
    const T = new Uint8Array(HMAC3.outputLen);
    for (let counter = 0; counter < blocks; counter++) {
      HKDF_COUNTER[0] = counter + 1;
      HMACTmp.update(counter === 0 ? EMPTY_BUFFER : T).update(info).update(HKDF_COUNTER).digestInto(T);
      okm.set(T, hash4.outputLen * counter);
  ...
  function hashPayload(payload) {
    const hash4 = sha2562(utf8Encoder.encode(JSON.stringify(payload)));
    return bytesToHex2(hash4);
  }
